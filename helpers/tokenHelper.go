...
func UpdateAllTokens(signedToken string, signedRefreshToken string, userId string) {
	timeoutContext, cancelFunc := context2.WithTimeout(context2.Background(), 100*time.Second)
	...
	_, err := userCollection.UpdateOne(timeoutContext, filter, bson.D{{"$set", updateObj}}, &opt)
	defer cancelFunc()
